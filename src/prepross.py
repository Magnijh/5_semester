#%%
import pandas as pd
from src.util import StandardScaler, DataLoader
import torch

# %%
# Change the parameters in the read_csv() function to match your csv-file, this is the data the model will be trained on.
df = pd.read_csv("CSV_Combiner\mergedData24h.csv", header=0, names=["date", "Oil", "Dollar"])
...
# How many data points the model uses to predict a new value
INPUT_LENGTH = 3
# How many data points the model predicts into the future
SEQ_LENGTH = 1
# The size of the batches the data will be split into (only change it if you're sure of the implication)
BATCH_SIZE = 16
...
#NUMBER OF FEATURES
NUM_NODES = 2
# If you want to make use of the adaptive adjacency matrix set to true (only relevant if the number of features is greater than 1)
ADDAPTADJ = True
# Only change the value of the variables below if you're sure of the implication
NHID = 256
DROPOUT = 0.3
WEIGHT_DECAY = 0.0001
LEARNING_RATE = 0.001
...
val_time = []
train_time = []
# The amount of the times you want to train on the entire dataset
EPOCHS = 75
SAVE = './garage/metr'
EXPID = 0
PRINT_EVERY = 10
for i in range(1, EPOCHS + 1):
    ...
    for iter, (x, y) in enumerate(dataloader['train_loader'].get_iterator()):
        ...
        if iter % PRINT_EVERY == 0:
            log = 'Iter: {:03d}, Train Loss: {:.6f}, Train MAPE: {:.4f}, Train RMSE: {:.4f}'
    ...
    log = 'Epoch: {:03d}, Train Loss: {:.6f}, Train MAPE: {:.4f}, Train RMSE: {:.4f}, Valid Loss: {:.6f}, Valid MAPE: {:.4f}, Valid RMSE: {:.4f}, Training Time: {:.4f}/epoch'
    print(log.format(i, mtrain_loss, mtrain_mape, mtrain_rmse, mvalid_loss, mvalid_mape, mvalid_rmse, (t2 - t1)), flush=True)
    torch.save(engine.model.state_dict(), SAVE + "_epoch_" + str(i) + "_" + str(round(mvalid_loss, 6)) + ".pth")
print("Average Training Time: {:.4f} secs/epoch".format(np.mean(train_time)))
print("Average Inference Time: {:.4f} secs".format(np.mean(val_time)))

#testing
bestid = np.argmin(his_loss)
engine.model.load_state_dict(torch.load(SAVE + "_epoch_" + str(bestid + 1) + "_" + str(round(his_loss[bestid], 6)) + ".pth"))
...
torch.save(engine.model.state_dict(), SAVE + "_exp" + str(EXPID) + "_best_" + str(round(his_loss[bestid], 6)) + ".pth")
# uncomment to also save a related scaler with your model
...
#%%
# Everything from this point on can potentially break if the number of timeseries is changed
# However, the code below is only used to visualize the precision of the model
Y_real = realy[:, 0, :, -1]
Y_pred = yhat[:, 0, :, -1]
#%%
